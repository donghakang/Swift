...
open class TRMosaicLayout: UICollectionViewLayout {
    ...
    func createCellLayout(withIndexPath index: Int, bigCellSide: Bool, cellBuffer: [IndexPath]) -> (Bool, [IndexPath]) {
        ...
        case .one:
            createOneCellLayout(withIndexPath: cellIndexPath)
        case .full:
            createFullCellLayout(withIndexPath: cellIndexPath)
    ...
    func createOneCellLayout(withIndexPath indexPath:IndexPath) {
        addOneCellLayout(atIndexPath: indexPath)
    }
    
    func createFullCellLayout(withIndexPath indexPath:IndexPath) {
        addFullCellLayout(atIndexPath: indexPath)
    }
    ...
    func addOneCellLayout(atIndexPath indexPath:IndexPath) {
        
        let cellHeight = layoutAttributes(withCellType: .one, indexPath: indexPath, atColumn: 0)
        
        columns[0].appendToColumn(withHeight: cellHeight)
        columns[1].appendToColumn(withHeight: cellHeight)
        columns[2].appendToColumn(withHeight: cellHeight)
    }
    
    func addFullCellLayout(atIndexPath indexPath:IndexPath) {
        
        let cellHeight = layoutAttributes(withCellType: .full, indexPath: indexPath, atColumn: 0)
